...
public class AuthenticationHelper {
    ...
    AuthenticationHelper(String userPoolID, String clientid, String secretKey) {
        ...
        try {
            ...
            this.accessKey = prop.getProperty("ACCESS_KEY");
            this.accessSecretKey = prop.getProperty("SECRET_KEY");
    ...
    String performSRPAuthentication(String username, String password) {
        String response = "";
        AdminInitiateAuthRequest authRequest = initiateUserSrpAuthRequest(username, password);
        AWSCredentials awsCreds = new BasicAWSCredentials(accessKey, accessSecretKey);
        ...
        if (authResult != null) {
            response = authResult.getAuthenticationResult().getIdToken();
        }
        return response;
    }
     ...
     */
    private AdminInitiateAuthRequest initiateUserSrpAuthRequest(String username, String password) {
        ...
        authRequest.addAuthParametersEntry("SECRET_HASH", this.calculateSecretHash(this.clientId,this.secretKey,username));
        authRequest.addAuthParametersEntry("USERNAME", username);
        authRequest.addAuthParametersEntry("PASSWORD", password);
//        authRequest.addAuthParametersEntry("SRP_A", this.getA().toString(16));
        return authRequest;
    }
     ...
     */
    private AdminRespondToAuthChallengeRequest userSrpAuthRequest(AdminInitiateAuthResult challenge, String password
    ) {
